...
# Calculate all as percentages of the Living income benchmark - the total of the income and gap
  summarise(Gap = median(LI BENCHMARK - NET TOTAL INCOME COLUMN)/LI BENCHMARK,
            Noncocoa = median(NET NONcocoa COLUMN + AT HOME FOOD INCOME COLUMN)/LI BENCHMARK,
            cocoa = median(NET cocoa INCOME COLUMN)/LI BENCHMARK %>% 
# Gather each income components into one column so the data is in 'long' format
  gather(key = "Component", value = "Income", Gap:cocoa) %>% 
# Re-level the income factors for the order you want them stacked on the graph  
  mutate(Component = factor(Component, 
...
all %>% 
  ...
  summarise(Gap = median(gap),
            Noncocoa = median(noncocoa_income_U),
            Food = median(Food_Income_U),
            cocoa = median(cocoa_income_net_U)) %>% 
  gather(key = "Component", value = "Income", Gap:cocoa) %>% 
  mutate(Component = factor(Component, levels = c("Gap", "Noncocoa", "Food", "cocoa"))) %>% 
...
# the median non-cocoa income, and the median cocoa income. 
  summarise(Gap = median(LI BENCHMARK - NET TOTAL INCOME COLUMN),
            Noncocoa = median(NET NONcocoa COLUMN),
            Food = median(AT HOME FOOD INCOME COLUMN),
            cocoa = median(NET cocoa INCOME COLUMN)) %>% 
# Gather each income components into one column so the data is in 'long' format
  gather(key = "Component", value = "Income", Gap:cocoa) %>% 
# Re-level the income factors for the order you want them stacked on the graph  
  mutate(Component = factor(Component, 
...
all %>% 
  ...
  group_by(Gender) %>% 
  summarise(Gap = median(gap)/
              median(gap + noncocoa_income_U + Food_Income_U + cocoa_income_net_U),
            Noncocoa = median(noncocoa_income_U)/
              median(gap + noncocoa_income_U + Food_Income_U + cocoa_income_net_U),
            Noncocoa = median(Food_Income_U)/
              median(gap + noncocoa_income_U + Food_Income_U + cocoa_income_net_U),
            cocoa = median(cocoa_income_net_U)/
              median(gap + noncocoa_income_U + Food_Income_U + cocoa_income_net_U)) %>% 
  gather(key = "Component", value = "Income", Gap:cocoa) %>% 
  mutate(Component = factor(Component, levels = c("Gap","Food", "Noncocoa", "cocoa"))) %>% 

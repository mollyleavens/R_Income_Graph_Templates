...
The following two code chunks allow the user to define graph colors and fonts respectively. These functionalities are helpful in matching the graphs to company/organization specific reports and presentations. 

These two code sections are OPTIONAL. If you would like to leave the graph colors as they are in the example graphs, no need to alter the following code chunk.  

## Colors 
```{r}
# You can communicate colors with R in two ways: through one of the 657 color names build into R, or through a hex color code. 
...
Set the font in this code chunk that you would like for all outputted graphs and data tables.

If you do not set the font, it will remain as Arial, the R default font.
...
below <- paste(below$grouping, ": ", below$Percent, " below", sep = "")
...
df %>%
  ...
  ggplot(aes(x = total_hh_income)) +
  # Add density line. You can change the fill and line colors.
  geom_smooth(color = "#add8e6", fill = "#add8e6") +
  # Add vertical line for living income
  geom_vline(aes(xintercept = mean(benchmark), 
                 color = "Living Income Benchmark"), key_glyph = "path") +
  # Add vertical line for mean income 

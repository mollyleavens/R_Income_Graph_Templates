...
When replacing the capitalized sections, retain the quotation marks. In other words, write your replacement within the quotation marks as this signals to R that you are not refferencing an object that already exists.   
...
DF$total_hh_income <- DF$ total_hh_income
```

# Graph aesthetics changes 

The following code chunk allows the user to define several graph aesthetics. 
...
Share of those below the Living Income Benchmark - distributional
plot
## By household type
# Female−headed
# Male−headed, large
...
theme_set(theme_get() + theme(text = element_text(family = 'mono')))
...
library(extrafont)
font_import()
# After running the font_import() function, you will likely be prompted with   # the following message: "Importing fonts may take a few minutes, depending   # on the number of fonts and the speed of the system. Continue? [y/n]"
# Type Y, then enter and the function should run.

fonttable() # Table outputs all available fonts. 
...
other_income <- 
total_hh_income <- DF$total_hh_income_2018
currency <- "USD"

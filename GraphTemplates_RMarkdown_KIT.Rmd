...
df$gap <- df$benchmark - df$total_hh_income
# Replace negative values (ie, above benchmark) with NA
df$gap <- ifelse(df$gap > 0, df$gap, NA)

# Calculate gap to benchmark with value of food produced at home included 
df$gap_with_food <- df$benchmark - df$total_hh_income_2018_including_food_value
# Replace negative values (ie, above benchmark) with NA
df$gap <- ifelse(df$gap > 0, df$gap, NA)
```

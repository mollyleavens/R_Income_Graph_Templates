...
Before continuing any further, load the R libraries that house the necessary functions for this analysis. If you have not previously used these R libraries, you will need to install them. See instructions in subsequent code chunk.  
...
# This last line of code does not relate to color. The line is in this code chunk to ensure
# it gets run and the default size gets assigned.
# See section "Bar chart label size" for more info.
bar_label_size <- 3
```
...
The size of the labels that identify the bar chart segments is currently set to 3. This is a relative measurement of size within R. If you would like larger labels, try increasing the size to between 4-8. If you would like smaller labels, try decreasing the size to 2. To change the size, simply replace the 3 in the subsequent line of code with another number. Then, re-run the subsequent line of code with the new number and re-load your graphs.  
```{r}
bar_label_size <- 3
```
...
# Add incomes to respective graph components  
  ...
  geom_label(mapping=aes(x=mean(benchmark), 
          # Long way of calculating graph maximum               
            ...
            label=paste((percent(sum(below_benchmark)/nrow(df))), 
                        "of the households earn an \n income below the benchmark")),
...
below <-
  ...
  # Formatting the strings as phrases with punctuation
      # The \n pastes the grouping on a new line in the graph label. 
      # You can remove the \n from the below code if you would like the label as 
      # one continuous line. However, this could affect the labels' x-axis 
      # orientation if the long label does not fit on the graph. 
 below$label <- paste(below$Percent, " below the Living Income Benchmark: \n", below$grouping, sep = "")

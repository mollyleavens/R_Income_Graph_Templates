...
* Option #1: Knit the entire R markdown file, stored in the project's [Github repository](https://github.com/mollyleavens/R_Income_Graph_Templates), into a single PDF. Before knitting the R markdown into a PDF, the user must take three **necessary** actions. First, ensure your data file is in your working directory (i.e., in the same folder on your computer and/or Github repository as where you have saved the R markdown). Second, ensure you have installed all R libraries under "Load R libraries" code chunk below. Third, enter data under "User-entered data" code chunk below.

* Option #2: Run each code chunk independently directly in the R markdown or by copying each chunk into a separate R file. Before running any of the graph-specific code chunks, run each set-up code chunk that is commented as necessary in the first line of the code chunk. 

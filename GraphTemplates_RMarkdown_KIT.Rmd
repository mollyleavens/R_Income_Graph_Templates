...
df$below_benchmark_with_food <- 
  ifelse((df$total_hh_income + df$food_value) > df$benchmark, 0, 1)
...
fgt_gap_with_food <- pmax((df$benchmark - (df$total_hh_income + df$food_value)), 0)/df$benchmark
```
...
# g1 <- df %>% filter(grouping == levels(df$grouping)[1])
# g2 <- df %>% filter(grouping == levels(df$grouping)[2])
# g3 <- df %>% filter(grouping == levels(df$grouping)[3])
# g4 <- df %>% filter(grouping == levels(df$grouping)[4])
...
# Set x-axis to total household income and color by household type  
  geom_density(aes(x = total_hh_income, color = grouping, fill = grouping), 
               alpha = 0.7) +
...
# Add vertical lines for benchmarks by group    
  geom_vline(data = g1,
    aes(xintercept = mean(g1$benchmark)), color = color_1, key_glyph = "path") +
  
  geom_vline(data = g2,
    aes(xintercept = mean(g2$benchmark)), color = color_2, key_glyph = "path") +
  
  geom_vline(data = g3,
    aes(xintercept = mean(g3$benchmark)), color = color_3, key_glyph = "path") +
  
  geom_vline(data = g4,
    aes(xintercept = mean(g4$benchmark)), color = color_4, key_glyph = "path") +

# Add text next to vertical lines for % below benchmarks by group
  geom_label_repel(data = g1, aes(x = mean(g1$benchmark),
                                  y = max(density(df$total_hh_income)$y)),
            label = paste( "Living income ", g1$grouping[1], ": ",
              percent(sum(g1$below_benchmark/nrow(g1))), " below", sep = ""),
            color = color_1, vjust=1, hjust=0) 

  # geom_label(data = g2,
  #   mapping=aes(x = mean(g2$benchmark), y = max(density(df$total_hh_income)$y),
  #           label = paste( "Living income ", g1$grouping[1], ": ",
  #             percent(sum(g1$below_benchmark/nrow(g1))), " below", sep = "")),
  #           color = color_2, vjust=1, hjust=0) +
  # 
  # geom_label(data = g3,
  #   mapping=aes(x = mean(g3$benchmark), y = max(density(df$total_hh_income)$y),
  #           label = paste( "Living income ", g1$grouping[1], ": ",
  #             percent(sum(g1$below_benchmark/nrow(g1))), " below", sep = "")),
  #           color = color_3, vjust=1, hjust=0) 
        
      #       geom_label(
      # mapping=aes(
      #       x = mean(df$benchmark[df$grouping == 
      #                               unique(df$grouping)[4]]),
      #       y = max(density(df$total_hh_income[df$grouping == 
      #                               unique(df$grouping)[4]])$y),
      #       label = paste(
      #         "Living income ",
      #         (unique(df$grouping)[4]),
      #         ": ",
      #         (percent(
      #         sum(df$below_benchmark[df$grouping == unique(df$grouping)[4]])/
      #         length(df$below_benchmark[df$grouping == unique(df$grouping)[4]]))),
      #         "below", sep = "")),
      #       color = color_1,
      #       vjust=1, hjust=0)
